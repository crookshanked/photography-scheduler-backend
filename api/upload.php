...
$startTimeValue = $rowData[$columnMap['start_time']] ?? null;
                ...
                $db->prepare(
                    "INSERT INTO schedule_imported (entry_id, date, start_time, end_time, reason, student_first_name, student_last_name, comments, source, unique_id, date_time_added) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())"
                )->execute([
                    $uuid,
                    $formattedDate,
                    $formattedStartTime,
                    ...
                    $uniqueFileName,
                    $rowData[$columnMap['unique_id']] ?? null,
                ]);
