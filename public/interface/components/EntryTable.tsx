...
const EntryTable: React.FC<EntryTableProps> = ({ entries, onCheckboxChange }) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      data: 'done',
      render: (data: boolean) => {
        return `<input type="checkbox" class="form-checkbox h-6 w-6 text-green-600 border-gray-400 rounded focus:ring-green-500" ${data ? 'checked' : ''} />`;
      },
      createdCell: (cell: HTMLElement, _cellData: any, rowData: Entry) => {
        const checkbox = cell.querySelector('input[type="checkbox"]');
        if (checkbox) {
          checkbox.addEventListener('change', (e) => {
            onCheckboxChange(e as any, rowData.entry_id);
          });
  ...
  return (
    <div className="overflow-x-auto">
      <DataTable
        ...
        onRowClick={
          (rowData: Entry) => handleRowClick(rowData.entry_id)
        }
      />
    </div>
  );
