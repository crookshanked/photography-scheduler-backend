...
if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true && isset($_REQUEST['donut']) && isset($donutKey) && $_REQUEST['donut'] == $donutKey){
...
Server response: <empty string> public:103:49
 -->
                ...
                <div class="card shadow-sm">
                    <div class="card-body">
                         <h2 class="card-title mb-4">Run Bash Script</h2>
                        <button id="runScriptBtn" class="btn btn-danger">Run Script</button>
                        <p class="mt-3 text-muted">Note: This will execute a predefined bash script on the server.</p>
                        <div id="scriptOutput" class="mt-3 p-2 bg-light border rounded" style="max-height: 200px; overflow-y: auto; font-family: monospace; font-size: 0.8em;">
                            <!-- Script output will be displayed here -->
                        </div>
                        <script>
                            document.getElementById('runScriptBtn').addEventListener('click', function() {
                                if (confirm('Are you sure you want to run the bash script? This action cannot be undone.')) {
                                    const scriptOutputDiv = document.getElementById('scriptOutput');
                                    scriptOutputDiv.innerHTML = 'Running script... Please wait.';
                                    fetch('run_script.php', { // Create a new PHP file for this
                                        method: 'POST',
                                        headers: {
                                            'Content-Type': 'application/json'
                                        },
                                        body: JSON.stringify({ action: 'run_bash_script' })
                                    })
                                    .then(response => response.json())
                                    .then(data => {
                                        if (data.status === 'success') {
                                            scriptOutputDiv.innerText = data.output;
                                        } else {
                                            scriptOutputDiv.innerText = 'Error: ' + data.message;
                                        }
                                    })
                                    .catch(error => {
                                        console.error('Error:', error);
                                        scriptOutputDiv.innerText = 'An error occurred while communicating with the server.';
                                    });
                                }
                            });
                        </script>
